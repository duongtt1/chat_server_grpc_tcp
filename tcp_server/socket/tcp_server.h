...
struct TCPServer {
	...
	auto listen(const std::string &iface, int port) -> bool;
...
public:
	...
	/// Collection of all sockets, sockets for incoming data, sockets for outgoing data and dead
	/// connections.
	std::vector<TCPSocket *> receive_sockets_, send_sockets_;

	/// Function wrapper to call back when data is available.
	std::function<void(TCPSocket *s)> recv_callback_ = nullptr;
	/// Function wrapper to call back when all data across all TCPSockets has been read and
