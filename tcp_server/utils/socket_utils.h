...
[[nodiscard]] inline auto createSocket(const SocketCfg &socket_cfg) -> int {
	...
	const int input_flags =
		(socket_cfg.is_listening_ ? AI_PASSIVE : 0) | (AI_NUMERICHOST | AI_NUMERICSERV);
	...
	const auto rc =
		getaddrinfo(ip.c_str(), std::to_string(socket_cfg.port_).c_str(), &hints, &result);
	...
	for (addrinfo *rp = result; rp; rp = rp->ai_next) {
		...
		if (socket_cfg.is_listening_) {
			...
			ASSERT(bind(socket_fd,
						socket_cfg.is_udp_ ? reinterpret_cast<const struct sockaddr *>(&addr)
										   : rp->ai_addr,
						sizeof(addr)) == 0,
